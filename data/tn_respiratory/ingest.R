...
result <- tryCatch({
  ...
  for (page_url in source_urls) {
    ...
    if (!is.null(html_dom)) {
      ...
      data_links <- data_links[!grepl("dict|dictionary|data.dict|methodology|readme|template|codebook|glossary|data.definition",
  ...
  names(data_raw) <- make.unique(tolower(gsub("[^a-z0-9]", "_", names(data_raw))))
  if (ncol(data_raw) > 1) {
