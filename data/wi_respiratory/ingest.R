...
source_urls <- c("https://www.dhs.wisconsin.gov/disease/laboratory-based-data.htm",
...
result <- tryCatch({
  resp_keywords <- c("influenza","rsv","covid","respiratory","positive","cases",
                     "surveillance","ili","ari","pcr","virus","percent","hosp",
                     "death","flu","pathogen","season","week","wastewater","concentration")
  all_data   <- list()
  ...
  for (page_url in source_urls) {
    ...
    html <- tryCatch(rvest::read_html(httr::content(resp, "text", encoding="UTF-8")),
      error=function(e) NULL)
    ...
    for (dl_url in unique(data_links)[seq_len(min(10, length(unique(data_links))))]) {
      ...
      local_file <- paste0("raw/wi_", length(all_data) + 1, ".", ext)
  ...
  if (length(all_data) == 0) stop("No CSV/Excel/JSON download links found on any WI DHS page")
  ...
  list(success=TRUE, rows=nrow(data_raw),
       message=paste("WI DHS:", nrow(data_raw), "rows from",
